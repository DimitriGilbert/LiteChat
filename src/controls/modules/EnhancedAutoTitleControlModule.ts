...
import { EnhancedAutoTitleControlTrigger } from "@/controls/components/auto-title/EnhancedAutoTitleControlTrigger";
...
export class EnhancedAutoTitleControlModule implements ControlModule {
  readonly id = "core-enhanced-auto-title";
  ...
  async initialize(modApi: LiteChatModApi): Promise<void> {
    ...
    const unsubContext = modApi.on(conversationEvent.selectedItemChanged, () => {
  ...
  private checkInteractionState() {
    ...
    const isFirst = conversationInteractions.length === 0;
    const isMultiple = conversationInteractions.length > 1;
  ...
  public updateConversationTitle = async (): Promise<void> => {
    ...
    try {
      ...
      console.error("[EnhancedAutoTitleControlModule] Error updating title:", error);
  ...
  register(modApi: LiteChatModApi): void {
    ...
    const triggerRenderer = () => {
      return React.createElement(EnhancedAutoTitleControlTrigger, { module: this });
    };
