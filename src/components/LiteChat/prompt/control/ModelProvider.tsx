...
export const ModelProviderControlComponent: React.FC = () => {
  const {
    selectedProviderId, selectedModelId, selectProvider, selectModel,
    getActiveProviders, dbProviderConfigs // Need configs to get models
  } = useProviderStore(useShallow(state => ({
    selectedProviderId: state.selectedProviderId, selectedModelId: state.selectedModelId,
    selectProvider: state.selectProvider, selectModel: state.selectModel,
    getActiveProviders: state.getActiveProviders, dbProviderConfigs: state.dbProviderConfigs,
  })));
  ...
  return (
    <div className='flex items-center gap-1'>
      <Select value={selectedProviderId ?? ''} onValueChange={(v) => selectProvider(v || null)}>
        <SelectTrigger className='h-8 text-xs w-[120px]'><SelectValue placeholder="Provider" /></SelectTrigger>
        <SelectContent>{activeProviders.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent>
      </Select>
       <Select value={selectedModelId ?? ''} onValueChange={(v) => selectModel(v || null)} disabled={!selectedProviderId}>
        <SelectTrigger className='h-8 text-xs w-[150px]'><SelectValue placeholder="Model" /></SelectTrigger>
        <SelectContent>{modelsForSelectedProvider.map(m => <SelectItem key={m.id} value={m.id}>{m.name || m.id}</SelectItem>)}</SelectContent>
      </Select>
    </div>
  );
};
...
export const useModelProviderControlRegistration = () => {
  ...
  const providerState = useProviderStore(useShallow(state => ({
  ...
  }, [register, providerState.isLoading, providerState.selectedProviderId, providerState.selectedModelId]);
};
