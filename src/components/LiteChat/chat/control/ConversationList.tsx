...
export const ConversationListControlComponent: React.FC = () => {
  const { conversations, selectConversation, selectedConversationId, addConversation, deleteConversation } = useConversationStore(
     useShallow(state => ({ // Use shallow comparison
        ...
        <ul className='space-y-0.5 p-1'>
          {conversations.map(c => (
            <li key={c.id}
              ...
              <Button variant='ghost' size='icon' className='h-5 w-5 opacity-0 group-hover:opacity-100 text-destructive hover:bg-destructive/10 flex-shrink-0' onClick={(e) => handleDelete(c.id, e)}>
