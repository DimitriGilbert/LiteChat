...
export function useAsyncOperation<TArgs extends any[], TResult>(
  ...
  options?: {
    setLoading?: (loading: boolean) => void; // Optional external loading setter
    setError?: (error: string | null) => void; // Optional external error setter
  ...
  const run = useCallback(
    async (...args: TArgs): Promise<TResult | undefined> => {
      ...
      try {
        ...
        setError(errorMsg); // Set local or external error state
