...
export const useInteractionStore = create(
  immer<InteractionState & InteractionActions>((set, get) => ({
    ...
    addInteraction: async (interactionData) => {
      const newIndex =
        get().interactions.reduce((max, i) => Math.max(max, i.index), -1) + 1;
      ...
      set((state) => {
        if (!state.interactions.some((i) => i.id === interaction.id)) {
          ...
          if (interaction.status === "STREAMING")
            state._addStreamingId(interaction.id);
        }
      });
      ...
      set((state) => {
        ...
        if (index !== -1) {
          ...
          if (updates.status === "STREAMING") state._addStreamingId(id);
            ...
            ["COMPLETED", "ERROR", "CANCELLED"].includes(updates.status ?? "")
          ) {
            state._removeStreamingId(id);
            if (updates.status === "ERROR")
              state.error = updates.metadata?.error || "Unknown error";
          }
        }
      });
      ...
      set((state) => {
        ...
        if (interaction?.status === "STREAMING") {
          if (
            ...
            console.warn(
              "Appending chunk to non-string response not implemented",
            );
        }
      });
    },
    setInteractionStatus: (id, status, error) => {
      get().updateInteraction(id, {
        status,
        metadata: {
          ...get().interactions.find((i) => i.id === id)?.metadata,
          error,
        },
      });
    },
    setCurrentConversationId: (id) => {
      if (get().currentConversationId !== id) {
        ...
        if (id) get().loadInteractions(id);
        else get().clearInteractions();
      }
    },
    clearInteractions: () =>
      set({
    ...
    _removeStreamingId: (id) =>
      set((state) => {
        ...
        if (state.streamingInteractionIds.length === 0)
